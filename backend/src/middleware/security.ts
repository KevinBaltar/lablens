...
function allowedOrigins(): string[] {
  return (process.env.CORS_ORIGIN || 'http://localhost:5173').split(',').map((origin) => origin.trim())
}
...
export function securityLogger(req: Request, _res: Response, next: NextFunction) {
  const sensitivePaths = ['/api/auth/login', '/api/auth/register', '/api/auth/forgot-password']
